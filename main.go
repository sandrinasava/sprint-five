...
func (r Running) Calories() float64 {
	...
	return (CaloriesMeanSpeedMultiplier*((float64(r.Action)*r.LenStep/MInKm)/r.Duration.Hours()) + CaloriesMeanSpeedShift) * r.Weight / (r.Duration.Minutes() / MinInHours) * MinInHours
}
...
func (w Walking) Calories() float64 {
	...
	speed := ((float64(w.Action) * w.LenStep / MInKm) / w.Duration.Hours() / KmHInMsec)
...
func (s Swimming) Calories() float64 {
	...
	return ((float64(s.LengthPool) * float64(s.CountPool) / MInKm / s.Duration.Hours()) + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * (s.Duration.Minutes() / MinInHours)
}
