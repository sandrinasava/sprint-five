...
func (t Training) meanSpeed() float64 {
	return (float64(t.Action) * t.LenStep / MInKm) / (t.Duration.Minutes() / MinInHours)
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	return (CaloriesMeanSpeedMultiplier*((float64(r.Action)*r.LenStep/MInKm)/(r.Duration.Minutes()/MinInHours)) + CaloriesMeanSpeedShift) * r.Weight / (r.Duration.Minutes() / MinInHours) * MinInHours
}
...
func (w Walking) Calories() float64 {
	speed := ((float64(w.Action) * w.LenStep / MInKm) / (w.Duration.Minutes() / MinInHours) / KmHInMsec)
	speedSquared := math.Pow(speed, 2)
	return (CaloriesWeightMultiplier*w.Weight + (speedSquared/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * (w.Duration.Minutes() / MinInHours) * MinInHours
}
...
func (s Swimming) Calories() float64 {
	return ((float64(s.LengthPool) * float64(s.CountPool) / MInKm / (s.Duration.Minutes() / MinInHours)) + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * (s.Duration.Minutes() / MinInHours)
}
